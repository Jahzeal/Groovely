...
import { 
  ...
  ArrowLeft, Check, Copy, ChevronDown, Plus, Upload, Loader2, UserX, UserMinus, X
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-white font-['Space_Grotesk',sans-serif] flex flex-col overflow-x-hidden">
      ...
      <header className="h-[76px] px-4 sm:px-8 border-b border-[#232B3E] bg-[#0F172A]/90 backdrop-blur-md flex items-center justify-between z-30">
        ...
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          <button 
            ...
            className="p-2 text-zinc-400 hover:text-white rounded-lg bg-[#192134] border border-[#2D3548] transition-colors cursor-pointer"
            title="Back"
          >
            <ArrowLeft size={18} />
          </button>
          ...
          {/* LIVE NOW Red Badge */}
          <div className="flex items-center gap-1.5 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-md bg-[#FF0044]/10 border border-[#FF0044]/30 text-[#FF0044] text-[10px] sm:text-xs font-bold uppercase tracking-wider shrink-0">
            <span className="w-2 h-2 rounded-full bg-[#FF0044] animate-ping" />
            <span>LIVE NOW</span>
          </div>

          {/* Listener Count */}
          <div className="hidden md:flex items-center gap-2 text-zinc-400 text-xs font-medium shrink-0">
        ...
        {/* Center: Room Title */}
        <h1 className="font-['Clash_Display',sans-serif] text-sm sm:text-lg md:text-xl font-bold text-white tracking-wide truncate max-w-[160px] sm:max-w-xs md:max-w-md text-center px-2">
          {room?.title || 'Midnight Lo-Fi & Crypto Talk'}
        </h1>

        {/* Right: Actions */}
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          {/* Chat Button (Opens Chat Modal Drawer) */}
          <button
            onClick={() => setIsMobileChatOpen(true)}
            className="px-3 py-2 bg-[#8A2BE2]/20 hover:bg-[#8A2BE2]/30 border border-[#8A2BE2]/50 text-accent-purple hover:text-white font-bold text-xs rounded-xl transition-all flex items-center gap-1.5 cursor-pointer relative"
            title="Open Room Chat"
          >
            <MessageSquare size={16} />
            <span className="hidden sm:inline">Live Chat</span>
            {messages.length > 0 && (
              <span className="w-5 h-5 bg-[#8A2BE2] text-white text-[10px] font-bold rounded-full flex items-center justify-center ml-0.5">
          ...
          <button
            ...
            className="px-3 sm:px-4 py-2 bg-[#192134] hover:bg-[#232B3E] border border-[#2D3548] text-white font-bold text-xs rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
          >
            <Share2 size={15} />
          ...
          <button
            ...
            className="px-3 sm:px-4 py-2 bg-[#FF0044] hover:bg-[#d60039] text-white font-bold text-xs rounded-xl transition-all shadow-[0_0_15px_rgba(255,0,68,0.4)] flex items-center gap-1.5 cursor-pointer shrink-0"
          >
            <PhoneOff size={15} />
            <span>{isHostOrCreator ? 'End Room' : 'Leave'}</span>
          </button>
        </div>
      </header>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 p-4 sm:p-8 max-w-[1600px] mx-auto w-full">
        ...
        {/* ── LEFT COLUMN (5 Cols): CURRENT PLAYING TRACK & PLAYER CONTROLS ── */}
        <div className="lg:col-span-5 flex flex-col items-center space-y-6 bg-[#0F172A] p-6 rounded-3xl border border-[#232B3E]">
        ...
        {/* ── RIGHT COLUMN (7 Cols): ON STAGE & LIVE LISTENERS GRID ── */}
        <div className="lg:col-span-7 flex flex-col space-y-8 bg-[#0F172A] p-6 rounded-3xl border border-[#232B3E]">
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-4 max-h-[460px] overflow-y-auto pr-1 custom-scrollbar">
              {roomListeners.length > 0 ? (
                ...
                <div className="col-span-full py-8 text-center bg-[#0F172A]/40 rounded-2xl border border-[#232B3E]">
      ...
      <button
        onClick={() => setIsMobileChatOpen(true)}
        className="fixed bottom-6 right-6 z-40 px-4 py-3 bg-[#8A2BE2] hover:bg-[#7823c9] text-white font-bold text-xs rounded-full shadow-[0_0_25px_rgba(138,43,226,0.7)] flex items-center gap-2 cursor-pointer transition-all active:scale-95 hover:scale-105"
      >
