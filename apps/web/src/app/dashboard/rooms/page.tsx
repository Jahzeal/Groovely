  ...
  useEffect(() => {
    ...
    socket.on('room_ended', (data: { roomId: number }) => {
      if (data?.roomId) {
        setRooms(prev => prev.map(room => 
          Number(room.id) === Number(data.roomId) 
            ? { ...room, status: 'ended', is_live: false } 
            : room
        ));
      }
    });
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-[#192134] border border-[#232B3E]">
                        ...
                        <button
                          ...
                          title="Cancel Scheduled Room"
                          className="absolute top-3 right-3 p-1.5 rounded-full bg-black/70 hover:bg-rose-600/90 backdrop-blur-md border border-white/10 hover:border-rose-500 text-zinc-400 hover:text-white transition-all cursor-pointer z-10"
                        >
                          <X size={14} />
                        </button>
